  ...
  <v-row class="h-100 w-100">
    <v-col class="h-100">
      ...
      <v-row class="button-container-wrapper">
        ...
        <v-col
          class="d-flex justify-center align-center flex-column button-container pt-12"
          cols="12"
          md="6"
          xl="4"
        >
          <v-btn block size="x-large" @click="openApplication"
            >NAKDİ YARDIM BAŞVURUSU</v-btn
          >
          ...
          <v-btn
            ...
            :key="ci"
            size="x-large"
            @click="applyForOneTimeAid(cat)"
            >{{ cat.name }} BAŞVURUSU</v-btn
          >
        </v-col>
      </v-row>
    </v-col>
...
const device = ref<any>({ name: "" });
  ...
  if (currentDevice != null) {
    ...
    console.log(useCookies(["device"]).get("device"));
...
async function setDeviceCookie() {
  try {
    const resp = await fetchGet(
      `/public/api/v1/mobile-device/${currentDevice}`
    );
